...
bar_colors = [colors[x] for x in alignment_counts.index]
...
ax2.scatter(involuntary["Election Year"], involuntary["Age in Election Year"],
            c='#e74c3c', label='Involuntary (Died)', alpha=0.7, s=80, marker='X', edgecolor='black')

# Add trend lines
if len(voluntary) > 1:
    z_vol = np.polyfit(voluntary["Election Year"], voluntary["Age in Election Year"], 1)
    p_vol = np.poly1d(z_vol)
    ax2.plot(voluntary["Election Year"].sort_values(), p_vol(voluntary["Election Year"].sort_values()),
...
if len(involuntary) > 1:
    ...
    ax2.plot(involuntary["Election Year"].sort_values(), p_inv(involuntary["Election Year"].sort_values()),
...
# Filter to opposing ideology departures
opposing_departures = departures[departures["Ideology Alignment"] == "Opposing"].copy()

# Determine congress control relative to justice's ideology
def get_opposing_congress_control(row):
    justice_ideology = row["Ideology"]
    house = row["House Control"]
    senate = row["Senate Control"]

    # For a liberal justice, "opposing" congress is Republican
    # For a conservative justice, "opposing" congress is Democratic
...
# Filter to aligned ideology departures
aligned_departures = departures[departures["Ideology Alignment"] == "Aligned"].copy()

# Determine congress control relative to justice's ideology (aligned = same party as justice)
def get_aligned_congress_control(row):
    justice_ideology = row["Ideology"]
    house = row["House Control"]
    senate = row["Senate Control"]

    # For a liberal justice, "aligned" congress is Democratic
    # For a conservative justice, "aligned" congress is Republican
    if justice_ideology == "Liberal":
...
plt.tight_layout()
plt.savefig('historical_departures_analysis.png', dpi=150, bbox_inches='tight')
plt.show()
...
print("\n\nChart saved to: historical_departures_analysis.png")
