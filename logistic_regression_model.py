...
# L1 regularization handles multicollinearity by shrinking redundant features to zero
...
# P(at least 1 leaves) = 1 - P(all stay) = 1 - (P(stay_1) * P(stay_2) * ... * P(stay_n))
prob_all_stay = 1.0
...
proba_sorted = [x[1] * 100 for x in sorted_data]  # Convert to percentage
...
for name in names_sorted:
    ...
    if alignment == 'Aligned':
        colors.append('#2ecc71')  # Green for aligned
    elif alignment == 'Opposing':
        colors.append('#e74c3c')  # Red for opposing
    else:
        colors.append('#f39c12')  # Orange for neutral
...
plt.tight_layout()
plt.savefig('2026_vacancy_predictions.png', dpi=150, bbox_inches='tight')
plt.show()
print("\nChart saved to: 2026_vacancy_predictions.png")
